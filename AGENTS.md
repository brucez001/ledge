...
- open sessions keep their `WKWebView` until the user closes them; the rail is
  restored across launches as rows only, and a restored row owns no
  `WKWebView` and fetches nothing until the user selects it, so launching at
